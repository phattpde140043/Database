...
source_table = "bronze_erp.suppliers"
target_table = "silver_database.suppliers"
...
merge_builder = (silver_table.alias("target").merge(bronze_dedup_df.alias("source"),"target."+primary_key+" = source."+primary_key)\
 .whenMatchedUpdate(
    ...
    set={
        ...
        "contact_name":col("source.contact_name"),
 ...
 .whenNotMatchedInsert(
     values={
        ...
        concat_ws(
            ...
            coalesce(col("source.contact_name"), lit("")),
            coalesce(col("source.phone"), lit("")),
            coalesce(col("source.email"), lit("")),
