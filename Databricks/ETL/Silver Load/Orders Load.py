...
source_table = "bronze_pos.orders"
target_table = "silver_database.orders"
...
merge_builder = (silver_table.alias("target").merge(bronze_dedup_df.alias("source"),"target."+primary_key+" = source."+primary_key)\
 .whenMatchedUpdate(
    ...
    set={
        ...
        "shipping_address":col("source.shipping_address"),
        "payment_type_id":col("source.payment_type_id"),
        "payment_status":col("source.payment_status"),
 ...
 .whenNotMatchedInsert(
     values={
        ...
        concat_ws(
            "|",
            coalesce(col("source.order_id").cast("string"), lit("")),
            coalesce(col("source.customer_id"), lit("")),
            coalesce(col("source.order_date").cast("string"), lit("")),
            coalesce(col("source.total_amount").cast("string"), lit("")),
            coalesce(col("source.shipping_address"), lit("")),
            coalesce(col("source.payment_type_id").cast("string"), lit("")),
            coalesce(col("source.payment_status"), lit("")),
