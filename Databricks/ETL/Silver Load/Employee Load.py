...
source_table = "bronze_erp.employees"
target_table = "silver_database.employees"
...
merge_builder = (silver_table.alias("target").merge(bronze_dedup_df.alias("source"),"target."+primary_key+" = source."+primary_key)\
 .whenMatchedUpdate(
    ...
    set={
        ...
        "department_id":col("source.department_id"),
 ...
 .whenNotMatchedInsert(
     values={
        ...
        concat_ws(
            "|",
            coalesce(col("source.employee_id"), lit("")),
            coalesce(col("source.name"), lit("")),
            coalesce(hex(col("source.email")), lit("")),
            coalesce(col("source.department_id").cast("string"), lit("")),
            coalesce(col("source.hire_date").cast("string"), lit("")),
            coalesce(col("source.salary").cast("string"), lit("")),
