...
source_table = "bronze_erp.purchase_order_items"
target_table = "silver_database.purchase_order_items"
...
merge_builder = (silver_table.alias("target").merge(bronze_dedup_df.alias("source"),"target."+primary_key+" = source."+primary_key)\
 .whenMatchedUpdate(
    ...
    set={
        "po_item_id":col("source.po_item_id"),
        "po_id":col("source.po_id"),
 ...
 .whenNotMatchedInsert(
     values={
        ...
        concat_ws(
            "|",
            coalesce(col("source.po_item_id").cast("string"), lit("")),
            coalesce(col("source.po_id").cast("string"), lit("")),
            coalesce(col("source.product_id"), lit("")),
            coalesce(col("source.quantity").cast("string"), lit("")),
            coalesce(col("source.unit_price").cast("string"), lit("")),
