...
source_table = "bronze_logistic.inventory"
target_table = "silver_database.inventory"
...
merge_builder = (silver_table.alias("target").merge(bronze_dedup_df.alias("source"),"target."+primary_key+" = source."+primary_key)\
 .whenMatchedUpdate(
    ...
    set={
        "inventory_id":col("source.inventory_id"),
        "warehouse_id":col("source.warehouse_id"),
 ...
 .whenNotMatchedInsert(
     values={
        ...
        concat_ws(
            "|",
            coalesce(col("source.inventory_id"), lit("")),
            coalesce(col("source.warehouse_id"), lit("")),
            coalesce(col("source.product_id"), lit("")),
            coalesce(col("source.sku_id"), lit("")),
            coalesce(col("source.stock_quantity"), lit("")),
            coalesce(col("source.last_updated").cast("string"), lit(""))
        ),
        256
    )
    }
