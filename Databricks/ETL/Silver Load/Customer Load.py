...
source_table = "bronze_pos.customers"
target_table = "silver_database.customers"
...
merge_builder = (silver_table.alias("target").merge(bronze_dedup_df.alias("source"),"target."+primary_key+" = source."+primary_key)\
 .whenMatchedUpdate(
    ...
    set={
        ...
        "ingestion_time":col("source.ingestion_time"),
 ...
 .whenNotMatchedInsert(
     values={
        ...
        concat_ws(
            ...
            coalesce(hex(col("source.phone")), lit("")),
            coalesce(col("source.address"), lit("")),
