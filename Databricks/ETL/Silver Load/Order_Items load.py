...
source_table = "bronze_pos.order_items"
target_table = "silver_database.order_items"
...
merge_builder = (silver_table.alias("target").merge(bronze_dedup_df.alias("source"),"target."+primary_key+" = source."+primary_key)\
 ...
 .whenNotMatchedInsert(
     values={
        ...
        concat_ws(
            "|",
            coalesce(col("source.order_item_id").cast("string"), lit("")),
            coalesce(col("source.order_id").cast("string"), lit("")),
            coalesce(col("source.product_id"), lit("")),
            coalesce(col("source.sku_id").cast("string"), lit("")),
