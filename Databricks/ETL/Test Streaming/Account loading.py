...
topic = "erpdbserver.public.accounts"
checkpoint_dir = "/path/to/checkpoint"
destination_table ="bronze_erp.accounts"
...
df_customers = df_json.filter(col("data.payload.op").isin("r", "c", "u")).select(
    ...
    col("data.payload.after.account_name").alias("account_name"),
    col("data.payload.after.account_type").alias("account_type"),
    ...
    sha2(
        concat_ws(
            "|",
            coalesce(col("data.payload.after.account_id").cast("string"), lit("")),
            coalesce(col("data.payload.after.account_name"), lit("")),
            coalesce(col("data.payload.after.account_type"), lit("")),
...
df_deduplicated = df_customers \
    ...
    .dropDuplicates(["account_id", "hash"])
