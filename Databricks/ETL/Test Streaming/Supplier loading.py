...
topic = "erpdbserver.public.suppliers"
checkpoint_dir = "/path/to/checkpoint"
destination_table ="bronze_erp.suppliers"
...
df_customers = df_json.filter(col("data.payload.op").isin("r", "c", "u")).select(
    ...
    col("data.payload.after.contact_name").alias("contact_name"),
    ...
    sha2(
        concat_ws(
            ...
            coalesce(col("data.payload.after.contact_name"), lit("")),
            coalesce(col("data.payload.after.phone"), lit("")),
            coalesce(col("data.payload.after.email"), lit("")),
