...
topic = "posdbserver.public.products_sku"
checkpoint_dir = "/path/to/checkpoint"
destination_table ="bronze_pos.products_sku"
...
df_customers = df_json.filter(col("data.payload.op").isin("r", "c", "u")).select(
    ...
    col("data.payload.after.sku").alias("sku"),
    col("data.payload.after.product_id").alias("product_id"),
    col("data.payload.after.color").alias("color"),
    col("data.payload.after.size").alias("size"),
    decode_decimal_udf(col("data.payload.after.price")).alias("price"),
    ...
    sha2(
        concat_ws(
            "|",
            coalesce(col("data.payload.after.sku_id").cast("string"), lit("")),
            coalesce(col("data.payload.after.sku"), lit("")),
            coalesce(col("data.payload.after.product_id"), lit("")),
            coalesce(col("data.payload.after.color"), lit("")),
            coalesce(col("data.payload.after.size"), lit("")),
            coalesce(col("data.payload.after.price").cast("string"), lit("")),
