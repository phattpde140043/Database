...
topic = "posdbserver.public.categories"
checkpoint_dir = "/path/to/checkpoint"
destination_table ="bronze_pos.categories"
...
df_customers = df_json.filter(col("data.payload.op").isin("r", "c", "u")).select(
    ...
    col("data.payload.after.category_id").alias("category_id"),
    ...
    sha2(
        concat_ws(
            "|",
            coalesce(col("data.payload.after.category_id").cast("string"), lit("")),
...
df_deduplicated = df_customers \
    ...
    .dropDuplicates(["category_id", "hash"])
