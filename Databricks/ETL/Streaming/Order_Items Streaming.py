...
topic = "posdbserver.public.order_items"
checkpoint_dir = "/path/to/checkpoint"
destination_table ="bronze_pos.order_items"
...
df_customers = df_json.filter(col("data.payload.op").isin("r", "c", "u")).select(
    ...
    col("data.payload.after.order_item_id").alias("order_item_id"),
    ...
    sha2(
        concat_ws(
            "|",
            coalesce(col("data.payload.after.order_item_id").cast("string"), lit("")),
...
df_deduplicated = df_customers \
    ...
    .dropDuplicates(["order_item_id", "hash"])
