...
topic = "erpdbserver.public.financial_transactions"
checkpoint_dir = "/path/to/checkpoint"
destination_table ="bronze_pos.financial_transactions"
...
df_customers = df_json.filter(col("data.payload.op").isin("r", "c", "u")).select(
    ...
    col("data.payload.after.transaction_id").alias("transaction_id"),
    col("data.payload.after.transaction_date").alias("transaction_date"),
    col("data.payload.after.account_id").alias("account_id"),
    decode_decimal_udf(col("data.payload.after.amount")).alias("amount"),
    col("data.payload.after.type").alias("type"),
    ...
    sha2(
        concat_ws(
            "|",
            coalesce(col("data.payload.after.transaction_id").cast("string"), lit("")),
            coalesce(col("data.payload.after.transaction_date").cast("string"), lit("")),
            coalesce(hex(col("data.payload.after.account_id")).cast("string"), lit("")),
            coalesce(hex(col("data.payload.after.amount")).cast("string"), lit("")),
            coalesce(col("data.payload.after.type"), lit("")),
...
df_deduplicated = df_customers \
    ...
    .dropDuplicates(["transaction_id", "hash"])
